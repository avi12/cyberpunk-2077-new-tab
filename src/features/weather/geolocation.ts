 ...
 *
 * City-level and no better, which is all the weather ever needed: the sky over a town is the same
 * question wherever inside it the coordinates land. It follows the connection rather than the
 * device, so a VPN moves it - which is why a reader is told where the reading came from and can type
 * over it.
 */
